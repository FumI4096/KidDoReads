...
function checkInputState() {
    ...
    const hasStartedEditing = getPassageTitle || getPassage || getAnswer || hasAudio || getChoices.some(choice => choice.value.trim() !== "");
...
ttsConvertButton.addEventListener("click", async () => {
    ...
    try {
        ...
        // Generate new speech
        await keyWordTtsObj.generateSpeech(`\(${passageTitle.value.trim()}\), \(${passage.value.trim()}\)`, ttsId.toString(), 5, voiceId);
        ...
        try{
            ...
            if (selectedContentId) {
                ...
                selectedContent.content_json.forEach((questionNo) => {
                    questionObject.push({
                        passageTitle: questionNo.passageTitle,
                        question: questionNo.question,
                        choices: questionNo.choices,
                        answer: questionNo.answer
                    });
                    console.log('Passage Title', questionNo.passageTitle);
